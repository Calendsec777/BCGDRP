...
from fusion_cfg import get_cfg_defaults
from fusion_net import BANDRPFusionCompare
...
def write_curve_csv(curve, out_path):
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["epoch", "train_mse", "val_mse", "test_mse"])
        w.writeheader()
        w.writerows(curve)
...
def parse_setting(setting):
    ...
    if setting == "none":
        return "none", "exp_drug"
    parts = setting.split("_")
    if len(parts) < 3:
        raise ValueError(f"Invalid setting: {setting}")
    if setting.startswith("concat_mlp_"):
        fusion_type = "concat_mlp"
        pair_suffix = setting[len("concat_mlp_"):]
    elif setting.startswith("cross_attn_"):
        fusion_type = "cross_attn"
        pair_suffix = setting[len("cross_attn_"):]
    elif setting.startswith("ban_"):
        fusion_type = "ban"
        pair_suffix = setting[len("ban_"):]
    else:
        raise ValueError(f"Unsupported setting: {setting}")

    if pair_suffix == "exp_drug":
        pair = "exp_drug"
    elif pair_suffix == "meth_drug":
        pair = "meth_drug"
    else:
        raise ValueError(f"Unsupported pair suffix in setting: {setting}")
    return fusion_type, pair


def run_one_setting(cfg, setting, train_loader, val_loader, test_loader, exp_dim, mut_dim, meth_dim, path_dim, device, optimizer_name, max_epochs, early_stop_patience, out_dir):
    cfg = cfg.clone()
    fusion_type, fusion_pair = parse_setting(setting)
    cfg["fusion"]["type"] = fusion_type
    cfg["fusion"]["pair"] = fusion_pair

    os.makedirs(out_dir, exist_ok=True)
    run_id = os.path.basename(out_dir)

    model = BANDRPFusionCompare(
    ...
    if optimizer_name == "adamw":
        optimizer = torch.optim.AdamW(model.parameters(), lr=float(cfg["model"]["lr"]), weight_decay=float(cfg["model"]["weight_decay"]))
    else:
        optimizer = torch.optim.Adam(model.parameters(), lr=float(cfg["model"]["lr"]), weight_decay=float(cfg["model"]["weight_decay"]))
    ...
    print(f"\n[{run_id}] setting={setting} | fusion_type={fusion_type} | fusion_pair={fusion_pair}")

    for epoch in range(max_epochs):
        train_mse = train_one_epoch(model, train_loader, optimizer, loss_fn, device)
        val_mse, _ = evaluate(model, val_loader, loss_fn, device)
        test_mse, _ = evaluate(model, test_loader, loss_fn, device)

        curve.append({"epoch": epoch, "train_mse": train_mse, "val_mse": val_mse, "test_mse": test_mse})
        print(f"  epoch={epoch:03d} train_mse={train_mse:.6f} val_mse={val_mse:.6f} test_mse={test_mse:.6f}")
    ...
    val_mse_best, val_metrics_best = evaluate(model, val_loader, loss_fn, device)
    test_mse_best, test_metrics_best = evaluate(model, test_loader, loss_fn, device)
...
def main():
    ...
    parser.add_argument(
        ...
        default=[
            ...
            "cross_attn_exp_drug",
            "ban_meth_drug",
            "concat_mlp_meth_drug",
            "cross_attn_meth_drug",
        ],
        help="Fusion comparison settings to run",
    )
    ...
    parser.add_argument("--ban_dropout", type=float, default=None)
    ...
    if args.ban_dropout is not None:
        cfg["fusion"]["ban_dropout"] = args.ban_dropout
    ...
    print(f"[FusionCompare] total settings = {len(args.settings)}")
    print(f"[FusionCompare] settings = {args.settings}")
    ...
    summary_csv = os.path.join(args.out_root, "fusion_compare_summary.csv")
    fields = [
        "run_id", "setting", "fusion_type", "fusion_pair", "best_epoch",
        "best_val_mse", "best_test_mse",
    ...
    all_results = []
    for idx, setting in enumerate(args.settings):
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        run_id = f"fusion_{idx:02d}_{setting}_{ts}"
    ...
    ranked_csv = os.path.join(args.out_root, "fusion_compare_summary_ranked.csv")
    ...
    print("\n===== Fusion comparison done =====")
    print(f"Summary saved to: {summary_csv}")
    print(f"Ranked summary saved to: {ranked_csv}")
    print("Top results by val_mse:")
    for r in ranked_results[:5]:
        print(
            f"  {r['setting']:>24s} | val_mse={r['best_val_mse']:.6f} | "
            f"test_mse={r['best_test_mse']:.6f} | test_pearson={r['test_pearson']:.4f}"
        )
